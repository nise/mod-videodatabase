			...
			if( this.options.allowCreation ){
				...
				var container = $('<div></div>')
					...
					.bind('click', {}, function(e){
						vi2.observer.log({context:'analysis',action:'add-marker',values:[ vi2.wp_user ]});		
						_this.addMarker();
					})
		...
		addMarker : function(){ 
			...
			if( ! _this.annotation_flag ){
				...
				var remove = $('.'+id+' .remove-btn')
					.click(function(e){
						vi2.observer.log({context:'analysis',action:'remove-marker',values:[ vi2.wp_user ]});
				...
				var save = $('.'+id+' .save-btn')
					.bind('click', {}, function(ee){
						vi2.observer.log({context:'analysis',action:'save-marker',values:[ vi2.wp_user ]}); 
						_this.annotation_flag = false; 
		...
		begin : function(e, id, obj){	  
			...
			if( this.currentMarker !== obj.data.id ){ 
				...
				var save = $('.'+obj.data.id + ' .save-btn')
					.bind('click', {}, function(ee){
						vi2.observer.log({context:'analysis',action:'edited-marker',values:[ vi2.wp_user ]});  
						// add new annotation to the DOM
						var data = {
							"type": _this.name,
							"id": obj.data.id,
